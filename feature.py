...
df = pd.read_csv('./data/after_clean_data/result_process02', sep=',')
df.dropna(axis=0, how='any', inplace=True)  # 按行删除空缺行

# 划分训练集和测试集
x_train, x_test, y_train, y_test = train_test_split(df[['jieba_cut_content']], df['label'], test_size=0.3,
                                                    random_state=0)
...
# 取出邮件内容部分
x_train = list(x_train['jieba_cut_content'].astype('str'))
print("====================================处理前的数据====================================")
print(x_train[199])
exit()
...
# 测试集进行TF—IDF转换
content_test = list(x_test['jieba_cut_content'].astype('str'))
