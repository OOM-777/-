...
def random_forest(X_TRAIN, Y_TRAIN, X_TEST, Y_TEST):
    print("======================开始训练随机森林模型======================")
    # 决策树数量、划分节点、最大深度
    forest = RandomForestClassifier(n_estimators=170, criterion='entropy', max_depth=3)
...
def SVM(X_TRAIN, Y_TRAIN, X_TEST, Y_TEST):
    print("======================开始训练SVM模型======================")
    svm = SVC(C=3, kernel='rbf')  # 惩罚参数、核函数
...
def logistic(X_TRAIN, Y_TRAIN, X_TEST, Y_TEST):
    print("======================开始训练逻辑回归模型======================")
    logist = LogisticRegression(penalty='l2',solver='sag')
    model=logist.fit(X_TRAIN,Y_TRAIN)
    joblib.dump(filename=LOGISTIC_PATH, value=model)
